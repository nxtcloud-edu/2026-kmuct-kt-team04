...
5. B는 지도·일정 화면, C는 채팅·AI를 담당합니다. C의 채팅 화면과 로컬 AI 연결은 통합했습니다. [연결 테스트 안내](AI-CONNECTION.md)를 참고하세요.
...
- Message 타입은 user/ai/system을 구분합니다. 서버 전용 `appendAssistantMessage`가 AI 메시지를 저장하며 userId는 요청 사용자입니다. 브라우저에 직접 노출하는 쓰기 API가 아닙니다.
- 로컬 개발에서는 `/api/ai/run`이 실제 대회 API 호출, 카카오 검색, 핀 저장, AI 메시지 저장을 수행합니다. AWS 환경에서 사용할 인증된 AI 실행 API 배포는 미완료입니다.
...
- 로컬 AI 서버는 실제 카카오 검색 결과에서 장소 ID·이름·좌표를 얻어 `createPin`에 전달합니다. 대회 API 키와 카카오 REST 키는 서버에서만 읽습니다.
...
로컬 추가 구현: 채팅 화면, 대회 AI 답변, 카카오 검색 기반 핀 생성, 파일 저장 및 로컬 이벤트 알림.
미완료: AWS 실제 배포, 이메일 인증 실제 동작, 서로 다른 로그인 사용자 간 실시간 통합 시험,
AWS AI 실행 API, 지도·일정 화면 통합, 삭제/탈퇴/강퇴, 여행 기간 변경, 과거 채팅 페이지, 경로/예산/사진.
